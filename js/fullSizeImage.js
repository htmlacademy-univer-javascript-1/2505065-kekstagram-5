...
const commentCountDisplay = bigPictureElement.querySelector(
...
const loadComments = (comments) => {
  const commentsToLoad = comments.slice(
    currentCommentsIndex,
    currentCommentsIndex + LOAD_STEP
  );
  ...
  commentCountDisplay.textContent = `${Math.min(
    currentCommentsIndex,
    comments.length
  )} из ${comments.length} комментариев`;
