...
function workDayLimit(dayStart, dayEnd, meetStart, meetTime) {
  ...
  switch (meetStart.length) {
    ...
    case 4 && meetEnd.indexOf(':') === 2:
      meetEnd =
        String(+meetStart.slice(0, 2) + Math.floor(meetTime / 60)) +
        ':' +
        String(+meetStart.slice(3) + (meetTime % 60));

      if (meetEnd.indexOf(':') === 1) {
        meetEnd = '0' + meetEnd;
      }

      if (+meetEnd.slice(3) >= 60) {
        meetEnd =
          String(+meetEnd.slice(0, 2) + 1) +
          ':' +
          String(+meetEnd.slice(3) % 60);
      }
      break;
    case 4 && meetEnd.indexOf(':') === 1:
    case 3:
      meetEnd =
        String(+meetStart.slice(0, 1) + Math.floor(meetTime / 60)) +
        ':' +
  ...
  if (meetStart.indexOf(':') === 1) {
    meetStart = '0' + meetStart;
  }

  if (dayStart.slice(0, 2) > meetStart.slice(0, 2)) {
    return false;
  } else if (
    dayStart.slice(0, 2) === meetStart.slice(0, 2) &&
    dayStart.slice(3) > meetStart.slice(3)
  ) {
    return false;
  }
  ...
  if (dayEnd.slice(0, 2) > meetEnd.slice(0, 2)) {
    return true;
  } else if (
    dayEnd.slice(0, 2) === meetEnd.slice(0, 2) &&
    dayEnd.slice(3) >= meetEnd.slice(3)
  ) {
    return true;
  } else {
    return false;
